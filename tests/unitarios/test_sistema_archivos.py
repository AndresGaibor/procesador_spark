...
def test_preparar_salida_local_configura_grupo_y_permisos(monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        "os.makedirs",
        lambda ruta, mode, exist_ok: llamadas.append(
            ("makedirs", ruta, mode, exist_ok)
        ),
    )
    monkeypatch.setattr("grp.getgrnam", lambda nombre: SimpleNamespace(gr_gid=777))
    monkeypatch.setattr(
        "os.chown", lambda ruta, uid, gid: llamadas.append(("chown", ruta, uid, gid))
    )
    monkeypatch.setattr(
        "os.chmod", lambda ruta, mode: llamadas.append(("chmod", ruta, mode))
    )
    ...
    assert capsys.readouterr().out == (
        "SALIDA_LOCAL_PREPARADA=/srv/talend-motor/salida/ventas modo_spark=append\n"
    )
