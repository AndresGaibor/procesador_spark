...
from motor_spark.dataflow_script.lexer import Token, Lexer
...
class ParserError(Exception):
    def __init__(
        self, mensaje: str, ubicacion: SourceLocation | None = None, codigo: str | None = None
    ) -> None:
...
class Parser:
    ...
    def _esperar(self, tipo: str, mensaje: str, codigo: str = "DFS_SYNTAX_EXPECTED") -> Token:
    ...
    def _parse_expresion_unaria(self) -> Expresion:
        ...
        if self._es_tipo("SIMBOLO") and self._actual().valor == "-":
            ...
            return Expresion(
                tipo=TipoExpresion.LITERAL_NUMERO,
                valor="-" + self._parse_expresion_unaria().valor,
                hijos=(operando,),
            )
    ...
    def _parse_expresion_primaria(self) -> Expresion:
        ...
        if self._es_tipo("PALABRA_RESERVADA") and self._tokens[self._pos + 1].valor == "(":
            return self._parse_columna_o_funcion()
    ...
    def _parse_columna_o_funcion(self) -> Expresion:
        ...
        if nombre.upper() in self.RESERVADAS_SIN_SOPORTE:
            self._reportar_error(
                f"Constructo no soportado: {nombre}",
                "DFS_UNSUPPORTED_CONSTRUCT"
            )
        ...
        if self._es_tipo("SIMBOLO") and self._actual().valor == "(":
            ...
            if nombre.upper() not in self.FUNCIONES_SOPORTADAS:
                self._reportar_error(
                    f"Funcion no soportada: {nombre}",
                    "DFS_UNSUPPORTED_FUNCTION"
                )

            return Expresion(
                tipo=TipoExpresion.FUNCION, valor=nombre, hijos=argumentos
            )
    ...
    def _parse_funcion_wrank(self) -> Expresion:
        ...
        if not (self._es_tipo("SIMBOLO") and self._actual().valor == ")"):
            ...
            while self._es_tipo("SIMBOLO") and self._actual().valor == ",":
                ...
                argumentos += (self._parse_expresion(),)
        self._esperar("SIMBOLO", "Se esperaba ')'", "DFS_SYNTAX_MISSING_RPAREN")
        return Expresion(tipo=TipoExpresion.WINDOW_RANK, valor="WRank", hijos=argumentos)
    ...
    def _parse_funcion_concat(self) -> Expresion:
        ...
        if not (self._es_tipo("SIMBOLO") and self._actual().valor == ")"):
            ...
            while self._es_tipo("SIMBOLO") and self._actual().valor == ",":
                ...
                argumentos += (self._parse_expresion(),)
        self._esperar("SIMBOLO", "Se esperaba ')'", "DFS_SYNTAX_MISSING_RPAREN")
        return Expresion(tipo=TipoExpresion.CONCATENACION, valor="CONCAT", hijos=argumentos)
    ...
    def _parse_condiciones_join(self) -> CondicionJoin | None:
        ...
        if self._es_valor("ON"):
            ...
            return CondicionJoin(
                izquierda=izquierda_col or "", derecha=derecha_col or "", tipo="=", es_natural=False
            )
    ...
    def _parse_select(self) -> SentenciaSelect:
        ...
        if self._es_valor("DISTINCT"):
            self._avanzar()
            self._reportar_error("SELECT DISTINCT no soportado", "DFS_UNSUPPORTED_DISTINCT")
        ...
        self._esperar("PALABRA_RESERVADA", "Se esperaba FROM", "DFS_SYNTAX_FROM_MISSING")
        ...
        if self._es_valor("GROUP"):
            ...
            if self._es_valor("BY"):
                ...
                self._reportar_error("Se esperaba BY despues de GROUP", "DFS_SYNTAX_BY_MISSING")
    ...
    def _parse_load(self) -> SentenciaLoad:
        self._avanzar()
        ...
        if self._es_valor("FROM"):
            ...
            self._avanzar()
            token_etiqueta = self._esperar("IDENTIFICADOR", "Se esperaba etiqueta RESIDENT", "DFS_SYNTAX_RESIDENT_EXPECTED")
            return SentenciaLoad(ruta="", expresion=None, campos=campos, distinct=distinct, es_resident=True, etiqueta_resident=token_etiqueta.valor, noconcatenate=noconcatenate)
        ...
        return SentenciaLoad(ruta=ruta, expresion=None, campos=campos, distinct=distinct, es_resident=False, noconcatenate=noconcatenate)
    ...
    def _parse_drop_table(self) -> SentenciaDropTable:
        self._avanzar()
        self._esperar("PALABRA_RESERVADA", "Se esperaba TABLE", "DFS_SYNTAX_TABLE_EXPECTED")
    ...
    def _parse_store(self) -> SentenciaStore:
        ...
        self._esperar("PALABRA_RESERVADA", "Se esperaba INTO", "DFS_SYNTAX_INTO_MISSING")

        if self._es_tipo("STRING"):
            lib_path = self._avanzar().valor.strip('"').strip("'")
        elif self._es_tipo("BRACKET_ID"):
            lib_path = self._avanzar().valor
        elif self._es_tipo("LIB_URI"):
            lib_path = self._avanzar().valor
        else:
            lib_path = ""
        ...
        return SentenciaStore(esquema="", tabla=tabla, ruta_destino=lib_path, formato=formato)
    ...
    def _parse_set(self) -> SentenciaSet:
        ...
        token_var = self._esperar("IDENTIFICADOR", "Se esperaba nombre de variable", "DFS_SYNTAX_VAR_EXPECTED")
        variable = token_var.valor

        self._esperar("SIMBOLO", "Se esperaba '='", "DFS_SYNTAX_EQ_MISSING")

        token_valor = self._esperar("STRING", "Se esperaba valor", "DFS_SYNTAX_VALUE_EXPECTED")
        valor = token_valor.valor.strip('"').strip("'")
    ...
    def _parse_lib_connect_to(self) -> SentenciaLibConnectTo:
        ...
        self._esperar("PALABRA_RESERVADA", "Se esperaba CONNECT", "DFS_SYNTAX_CONNECT_MISSING")
        ...
        if self._es_tipo("STRING"):
            ...
            self._reportar_error("Se esperaba nombre de conexion", "DFS_SYNTAX_CONN_EXPECTED")
            conexion = ""
    ...
    def _parse_sentencia(self) -> (
        ...
        if self._es_valor("SELECT"):
        ...
        elif self._es_valor("CONCATENATE") or self._es_valor("NOCONCATENATE"):
            ...
            self._reportar_error(f"Sentencia desconocida: {token.valor}", "DFS_SYNTAX_UNKNOWN_SENTENCE")
            self._avanzar()
            raise ParserError(f"Sentencia desconocida: {token.valor}", self._crear_ubicacion_actual(), "DFS_SYNTAX_UNKNOWN_SENTENCE")

    def _parse_etiqueta(self) -> Etiqueta:
        token_nombre = self._esperar("IDENTIFICADOR", "Se esperaba nombre de etiqueta", "DFS_SYNTAX_LABEL_NAME_EXPECTED")
        ...
        while not self._es_tipo("FIN") and not (
            self._es_tipo("IDENTIFICADOR") and self._tokens[self._pos + 1].valor == ":"
            if self._pos + 1 < len(self._tokens) else False
        ):
            if self._es_valor("SET") or self._es_valor("LIB"):
                self._reportar_error("SET y LIB CONNECT TO no son validos dentro de etiquetas", "DFS_UNSUPPORTED_GLOBAL_IN_LABEL")
                break
            ...
            if self._es_valor("SELECT") or self._es_valor("LOAD") or self._es_valor(
                "RESIDENT"
            ) or self._es_valor("DROP") or self._es_valor("STORE") or self._es_valor(
                "CONCATENATE"
            ) or self._es_valor(
                "NOCONCATENATE"
            ):
    ...
    def _parse_sentencia_para_etiqueta(
        ...
        if self._es_valor("SELECT"):
            return self._parse_select()
        elif self._es_valor("LOAD") or self._es_valor("NOCONCATENATE"):
            return self._parse_load()
        elif self._es_valor("RESIDENT"):
            return self._parse_resident()
        elif self._es_valor("DROP"):
            ...
            raise ParserError(f"Sentencia desconocida: {self._actual().valor}", self._crear_ubicacion_actual(), "DFS_SYNTAX_UNKNOWN_SENTENCE")
        ...
        while not self._es_tipo("FIN"):
            if self._es_valor("SET"):
                ...
                if (
                    ...
                    self._avanzar()
            elif self._es_valor("SELECT") or self._es_valor("LOAD") or self._es_valor(
                "RESIDENT"
            ) or self._es_valor("DROP") or self._es_valor("STORE") or self._es_valor(
                "CONCATENATE"
