...
class Normalizador:
    ...
    def quitar_bom(self) -> Normalizador:
        if self._procesado.startswith(BOM):
            self._procesado = self._procesado[len(BOM) :]
        return self
    ...
    def _es_dentro_bracket(self, texto: str, posicion: int) -> bool:
        balance: int = 0
        for i, char in enumerate(texto):
            if i >= posicion:
                break
    ...
    def quitar_comentarios(self) -> Normalizador:
        def reemplazar_comentario_linea(match: re.Match[str]) -> str:
            inicio = match.start()
            fin = match.end()
            if self._es_dentro_string(self._procesado, inicio) or self._es_dentro_bracket(
                self._procesado, inicio
            ):
        ...
        self._procesado = PATRON_COMENTARIO_LINEA.sub(reemplazar_comentario_linea, self._procesado)
        self._procesado = PATRON_COMENTARIO_BLOQUE.sub(reemplazar_comentario_bloque, self._procesado)
        return self
